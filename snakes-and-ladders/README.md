...
read back, if it exists, before being rewritten. During long runs a progress line is
printed, and the file freshly written, every minute, or whatever `--progress` is set to,
so little is lost should the run die. Naming several files skips the benchmark entirely:
here the results in `A.json` and `B.json` are merged into `C.json`, leaving the first two
files untouched, and no games are played at all:
