...
func newMergeCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runMerge(cmd.Context(), logs, databasePath, args[0], progressInterval)
		},
	}
...
// runMerge streams a CSV of "password,count" rows into the output database,
...
func runMerge(ctx context.Context, logs logging, dbPath, csvPath string, interval time.Duration) (err error) {
	...
	rep := startReporter(interval, counters.reportTo(logs.console, logs.file))
